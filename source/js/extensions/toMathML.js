MathJax.Hub.Register.LoadHook("[MathJax]/jax/element/mml/jax.js", function () {
  ...
  MML.mbase.Augment({
    ...
    toMathML: function (space) {
      ...
      if (data.length === 0 || (data.length === 1 && data[0] === "")) { return space + "<" + tag + attr + " />" }
        ...
        for (var id in defaults) {
          if (!skip[id] && defaults.hasOwnProperty(id)) {
            ...
            if (this[id] != null && (force || this[id] !== defaults[id])) {
              ...
              if (force || this.Get(id) !== value) { attr.push(id + '="' + this.toMathMLattribute(value) + '"') }
              this[id] = value;
            }
          }
        }
    ...
    toMathMLclass: function (attr) {
      ...
      if (this.mathvariant && this.toMathMLvariants[this.mathvariant]) { CLASS.push("MJX" + this.mathvariant) }
  ...
  MML.msubsup.Augment({
    toMathML: function (space) {
      ...
      var data = [];
      for (var i = 0, m = this.data.length; i < m; i++) { if (this.data[i]) { data.push(this.data[i].toMathML(space + "  ")) } }
      return space + "<" + tag + attr + ">\n" + data.join("\n") + "\n" + space + "</" + tag + ">";
    }
  });
