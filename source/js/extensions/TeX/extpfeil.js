...
MathJax.Hub.Register.StartupHook('TeX Jax Ready', function() {
  ...
  b.Parse.Augment({
    NewExtArrow: function(c) {
      var e = this.GetArgument(c),
        f = this.GetArgument(c),
        d = this.GetArgument(c);
      if (!e.match(/^\\([a-z]+|.)$/i)) {
        b.Error([
          'NewextarrowArg1',
      ...
      f = f.split(',');
      d = parseInt(d);
      this.setDef(e, ['xArrow', d, parseInt(f[0]), parseInt(f[1])]);
    }
  });
