...
MathJax.Extension['TeX/autoload-all'] = { version: '2.7.5' };
MathJax.Hub.Register.StartupHook('TeX Jax Ready', function() {
  ...
  var d,
    g,
    b,
    a = { macros: {}, environment: {} };
  for (d in h) {
    if (h.hasOwnProperty(d)) {
      if (!MathJax.Extension['TeX/' + d]) {
        var f = h[d];
        for (g = 0, b = f.length; g < b; g++) {
          a.macros[f[g]] = ['Extension', d];
        }
      }
    }
  }
    ...
    if (c.hasOwnProperty(d)) {
      if (!MathJax.Extension['TeX/' + d]) {
        ...
        for (g = 0, b = e.length; g < b; g++) {
          a.environment[e[g]] = ['ExtensionEnv', null, d];
        }
      }
    }
  }
  MathJax.InputJax.TeX.Definitions.Add(a);
