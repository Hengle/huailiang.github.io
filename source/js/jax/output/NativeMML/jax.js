  ...
  nMML.Augment({
    ...
    MSIEevent: function () {
      ...
      if (event.srcElement.className === "MathJax_MathPlayer_Overlay" && this.msieMath.fireEvent) {
        ...
        if (type === "ContextMenu" || type === "Mouseover" || type === "Mouseout") { this.msieMath.fireEvent("on" + event.type, event) }
      }
    ...
    MSIE9event: function (event) {
      ...
      if (event.type === "click" && nMML.settings.mpContext) { return EVENT.AltContextMenu(event, this) }
  ...
  HUB.Register.StartupHook("mml Jax Ready", function () {
    ...
    MML.mbase.Augment({
      ...
      NativeMMLattribute: function (value) {
        ...
        else if (value.match(/^\s*(([-+])?(\d+(\.\d*)?|\.\d+))\s*mu\s*$/)) { value = ((1 / 18) * RegExp.$1).toFixed(3).replace(/\.?0+$/, "") + "em" } // FIXME:  should take scriptlevel into account
