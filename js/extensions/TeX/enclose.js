...
MathJax.Hub.Register.StartupHook('TeX Jax Ready', function() {
  var c = MathJax.InputJax.TeX,
    ...
    b = MathJax.Extension['TeX/enclose'].ALLOWED;
  c.Definitions.Add({ macros: { enclose: 'Enclose' } }, null, true);
  c.Parse.Augment({
    Enclose: function(g) {
      var k = this.GetArgument(g),
        e = this.GetBrackets(g),
        j = this.ParseArg(g);
      var l = { notation: k.replace(/,/g, ' ') };
      if (e) {
        e = e.replace(/ /g, '').split(/,/);
        for (var h = 0, d = e.length; h < d; h++) {
          ...
          if (b[f[0]]) {
            ...
            if (f[0] === 'arrow' && f[1]) {
              l.notation = l.notation + ' updiagonalarrow';
            } else {
              l[f[0]] = f[1];
            }
          }
