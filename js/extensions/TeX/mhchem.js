...
if (MathJax.Extension['TeX/mhchem']) {
  ...
  MathJax.Extension['TeX/mhchem'] = {
    version: '2.7.5',
    config: MathJax.Hub.CombineConfig('TeX.mhchem', { legacy: true })
  };
  if (!MathJax.Extension['TeX/mhchem'].config.legacy) {
    ...
    MathJax.Hub.Register.StartupHook('TeX Jax Ready', function() {
      ...
      b.Parse.Augment({
        ...
        CEprescripts: function(f) {
          var d = this.ParseArg(f),
            i = this.ParseArg(f),
            h = this.ParseArg(f),
            g = this.ParseArg(f),
            e = this.ParseArg(f);
          var c = MathJax.ElementJax.mml;
          this.Push(c.mmultiscripts(h, g, e, c.mprescripts(), d, i));
        },
        CEnone: function(c) {
