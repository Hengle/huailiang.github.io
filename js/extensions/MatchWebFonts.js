...
(function(c, b) {
  ...
  var a = MathJax.Hub.CombineConfig('MatchWebFonts', {
    matchFor: { 'HTML-CSS': true, NativeMML: true, SVG: true },
    fontCheckDelay: 500,
      ...
      checkFonts: function(k, o) {
        ...
        for (p = 0, l = o.length; p < l; p++) {
          ...
          if (q !== h.HTMLCSS.ex || j !== h.HTMLCSS.em) {
            ...
            n = Math.floor(
              Math.max(this.config.minScaleAdjust / 100, n) * this.config.scale
            );
            if (n / 100 !== h.scale) {
              s.push(script);
              o[p] = {};
      ...
      checkFonts: function(A, l) {
        ...
        for (w = 0, s = l.length; w < s; w++) {
          ...
          if (r || h != g.mex) {
            ...
            C = Math.floor(
              Math.max(this.config.minScaleAdjust / 100, C) * this.config.scale
            );
            if (C / 100 !== g.scale) {
              o.push([v.style, C]);
            }
          ...
          if (
            'scrollWidth' in g &&
            (r || g.scrollWidth !== k.firstChild.scrollWidth)
          ) {
