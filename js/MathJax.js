  ...
  if (!MathJax.Hub) {  // skip if already loaded
    ...
    (function (BASENAME) {
      ...
      if (!BUGTEST.prototype.bug_test) {
        CONSTRUCTOR = function () { return function () { return arguments.callee.Init.call(this, arguments) }; };
      };
      ...
      var USING = function (args, i) {
        if (arguments.length > 1) {
          if (arguments.length === 2 && !(typeof arguments[0] === 'function') &&
            arguments[0] instanceof Object && typeof arguments[1] === 'number') { args = [].slice.call(args, i) }
            ...
            if (styles.hasOwnProperty(id)) {
              if (typeof styles[id] === 'string') {
                ...
                for (var name in styles[id]) {
                  if (styles[id].hasOwnProperty(name)) {
                    if (styles[id][name] != null) { style[style.length] = name + ': ' + styles[id][name] }
                  }
                }
      ...
      Element: function (type, def, contents) {
        ...
        if (def) {
          if (def.style) {
            ...
            for (var id in style) {
              if (style.hasOwnProperty(id)) { def.style[id.replace(/-([a-z])/g, this.ucMatch)] = style[id] }
            }
          }
    ...
    MathJax.Hub = {
      config: {
        ...
        errorSettings: {
          message: ["[Math Processing Error3]"],
      ...
      getJaxByType: function (type, element) {
        ...
        for (var i = 0, m = scripts.length; i < m; i++) {
          if (scripts[i].MathJax && scripts[i].MathJax.elementJax &&
            scripts[i].MathJax.elementJax.mimeType === type) { jax.push(scripts[i].MathJax.elementJax) }
        }
        return jax;
      },
    ...
    MathJax.Hub.Startup = {
      ...
      Jax: function () {
        ...
        for (var i = 0, m = config.jax.length, k = 0; i < m; i++) {
          if (config.jax[i].substr(0, 7) === "output/") { jax.order[config.jax[i].substr(7)] = k; k++ }
        }
        var queue = MathJax.Callback.Queue();
        return queue.Push(
      ...
      Menu: function () {
        ...
        if (registered && registered.length) {
          if (menu.renderer && menu.renderer !== registered[0].id) { registered.unshift(MathJax.OutputJax[menu.renderer]) }
      ...
      var JAX = MathJax.Object.Subclass({
        ...
        Augment: function (def, cdef) {
          ...
          if (def != null) {
            for (var id in def) {
              if (def.hasOwnProperty(id)) {
                if (typeof def[id] === "function") { cObject.protoFunction(id, def[id]) } else { ndef[id] = def[id] }
              }
            }
