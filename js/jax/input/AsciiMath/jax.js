...
(function (ASCIIMATH) {
  ...
  var DOCFRAG = MathJax.Object.Subclass({
    ...
    removeChild: function (node) {
      for (var i = 0, m = this.childNodes.length; i < m; i++) { if (this.childNodes[i] === node) break }
      if (i === m) return;
    ...
    replaceChild: function (node, old) {
      for (var i = 0, m = this.childNodes.length; i < m; i++) { if (this.childNodes[i] === old) break }
  ...
  var INITASCIIMATH = function () {
    ...
    MML.mbase.Augment({
      ...
      removeChild: function (node) {
        ...
        if (i) { this.childNodes[i - 1].nextSibling = node.nextSibling }
        this.nodeValue = "";
        for (i = 0, m = this.childNodes.length; i < m; i++) { this.nodeValue += this.childNodes[i].nodeValue }
        node.nextSibling = node.parent = null;
        return node;
      },
      replaceChild: function (node, old) {
  ...
  */
  ...
  (function () {
    for (var i = 0, m = AMsymbols.length; i < m; i++) {
      ...
      if (AMsymbols[i].atname === "fontfamily" || AMsymbols[i].atname === "fontweight") { AMsymbols[i].atname = "mathvariant" }
    }
  })();
