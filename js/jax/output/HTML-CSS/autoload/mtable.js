  ...
  MML.mtable.Augment({
    toHTML: function (span) {
      ...
      if (values.equalrows) {
        ...
        for (i = 0, m = A.length; i < m; i++) { s = ((Hm + Dm) - (H[i] + D[i])) / 2; H[i] += s; D[i] += s }
      }
      ...
      if (values.align.match(/(top|bottom|center|baseline|axis)( +(-?\d+))?/)) { n = RegExp.$3; values.align = RegExp.$1 } else { values.align = this.defaults.align }
      if (n !== "") {
        //
      ...
      if (values.equalcolumns && values.width !== "auto") {
        ...
        for (i = 0, m = Math.min(J + 1, CWIDTH.length); i < m; i++) {
          ...
          else if (CWIDTH[i].match(/%$/)) { P[p] = i; p++; Wp += W[i]; WP += HTMLCSS.length2em(CWIDTH[i], mu, 1) }
      ...
      if (hasRelativeWidth) {
        ...
        for (j = 0; j <= J; j++) {
          ...
          if (CWIDTH[j] !== "auto" && CWIDTH[j] !== "fit") { C[j].bbox.width = W[j]; C[j].bbox.isFixed = true }
